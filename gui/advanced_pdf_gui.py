...
class AdvancedPDFViewer(tk.Tk):
    ...
    def __init__(self):
        ...
        """
        super().__init__()
        self._photo_image_ref = None # Keep reference to PhotoImage preventing garbage collection

        self.title("Advanced PDF Viewer")
        self.geometry("1024x768")

        # Initialize PDF Engine
        self.engine = PdfEngine() # Initial instantiation using PdfEngine directly
        ...
        sample_pdf_paths = ["PDFs/9702_m23_qp_12.pdf", "PDFs/9702_m23_qp_22.pdf"]
        ...
        self.status_bar_frame = ttk.Frame(self, relief=tk.GROOVE, borderwidth=2)
        ...
        self.v_scrollbar = ttk.Scrollbar(self.display_frame, orient=tk.VERTICAL)
        self.h_scrollbar = ttk.Scrollbar(self.display_frame, orient=tk.HORIZONTAL)

        self.display_canvas = tk.Canvas(self.display_frame, bg="lightgray",  # Changed bg for visibility
        ...
        self.canvas_image_item = self.display_canvas.create_image(0, 0, anchor=tk.NW, image=None)

        # --- Controls Frame ---
        # PDF Navigation Buttons
        self.prev_pdf_button = ttk.Button(self.controls_frame, text="Previous PDF (Ctrl-Alt-H)", command=self.previous_pdf_file)
        self.prev_pdf_button.pack(fill=tk.X, padx=5, pady=2)

        self.next_pdf_button = ttk.Button(self.controls_frame, text="Next PDF (Ctrl-Alt-L)", command=self.next_pdf_file)
        ...
        self.prev_item_button = ttk.Button(self.controls_frame, text="Previous Item (Ctrl-Alt-K)", command=self.previous_item)
        self.prev_item_button.pack(fill=tk.X, padx=5, pady=2)

        self.next_item_button = ttk.Button(self.controls_frame, text="Next Item (Ctrl-Alt-J)", command=self.next_item)
        self.next_item_button.pack(fill=tk.X, padx=5, pady=2)

        ttk.Separator(self.controls_frame, orient=tk.HORIZONTAL).pack(fill=tk.X, pady=10)

        # Mode Switching Buttons
        self.page_mode_button = ttk.Button(self.controls_frame, text="View Pages (Ctrl+P)", command=self.switch_to_page_mode)
        self.page_mode_button.pack(fill=tk.X, padx=5, pady=2)

        self.question_mode_button = ttk.Button(self.controls_frame, text="View Questions (Ctrl+Q)", command=self.switch_to_question_mode)
        self.question_mode_button.pack(fill=tk.X, padx=5, pady=2)

        ttk.Separator(self.controls_frame, orient=tk.HORIZONTAL).pack(fill=tk.X, pady=10)

        # Debugging Button
        self.debug_button = ttk.Button(self.controls_frame, text="Debug Item (Ctrl+D)", command=self.debug_current_item)
        ...
        # Reloading Button
        self.reload_button = ttk.Button(self.controls_frame, text="Reload Engine (Ctrl+R)", command=self.reload_engine_code)
        self.reload_button.pack(fill=tk.X, padx=5, pady=2)

        # --- Status Bar Frame ---
        self.status_bar_text = tk.StringVar()
        self.status_bar_label = ttk.Label(self.status_bar_frame, textvariable=self.status_bar_text, relief=tk.SUNKEN, anchor=tk.W, wraplength=800, justify=tk.LEFT) # wraplength and justify for long messages
        ...
        self.bind('<Control-d>', lambda event: self.debug_current_item(combined_debug=False))
        self.bind('<Control-Shift-D>', lambda event: self.debug_current_item(combined_debug=True))
        self.bind('<Control-r>', self.reload_engine_code)

        # --- Initial Load ---
        self.update_status_bar("Welcome! Load a PDF to begin.") # Initial status
    ...
    def update_status_bar(self, general_message: str = ""):
        ...
        """
        file_info = "File: None"
        if self.engine and hasattr(self.engine, 'pdf_path') and self.engine.pdf_path: # Check pdf_path exists
        ...
        if self.navigation_mode == "page":
            if self.engine.current_pdf_document and self.total_pages > 0: # Check if PDF is loaded for page info
                item_info = f"Page: {self.current_page_number}/{self.total_pages}"
            else:
                item_info = "Page: N/A"
        elif self.navigation_mode == "question":
            if self.engine.current_pdf_document and self.total_questions > 0: # Check if PDF is loaded for q info
        ...
        full_status = " | ".join(filter(None, status_parts)) # filter(None,...) to remove empty strings if item_info is empty
    ...
    def render_current_page_or_question(self):
        ...
        if not self.engine.current_pdf_document or not self.engine.get_current_file_path():
        ...
        try:
            if self.navigation_mode == "page":
                if self.current_page_number > 0 and self.total_pages > 0:
                    self.update_status_bar(f"Rendering Page {self.current_page_number}/{self.total_pages}...")
                    surface = self.engine.render_pdf_page(self.current_page_number, debug=0)
                    general_render_message = "Page displayed." if surface else "Failed to render page."
                else:
                    general_render_message = "No page selected or PDF has no pages."
            elif self.navigation_mode == "question":
                if self.current_question_number > 0 and self.total_questions > 0 and self.questions_list:
                    self.update_status_bar(f"Rendering Question {self.current_question_number}/{self.total_questions}...")
                    surface = self.engine.render_a_question(self.current_question_number)

                    if surface is None and self.current_question_number -1 < len(self.questions_list) : # Fallback
                        current_q = self.questions_list[self.current_question_number - 1]
                        if current_q.pages:
                             page_to_render = current_q.pages[0]
                             self.update_status_bar(f"Fallback: Rendering Page {page_to_render} for Q{self.current_question_number}.")
                             surface = self.engine.render_pdf_page(page_to_render, debug=0)
                    general_render_message = "Question displayed." if surface else "Failed to render question."
                    if surface is None: general_render_message += " (Not available/Fallback failed)"
                else:
                    general_render_message = "No question selected or no questions available."

            print(general_render_message) # Print what was attempted/result

            if surface:
                self._photo_image_ref = self.convert_cairo_surface_to_photoimage(surface)
                self.display_canvas.itemconfig(self.canvas_image_item, image=self._photo_image_ref)
                self.display_canvas.coords(self.canvas_image_item, 0, 0)
                self.display_canvas.config(scrollregion=self.display_canvas.bbox(self.canvas_image_item))
            else:
                self.display_canvas.itemconfig(self.canvas_image_item, image=None)
                self._photo_image_ref = None
                self.display_canvas.config(scrollregion=(0,0,0,0))

            self.update_status_bar(general_render_message) # Final status update
    ...
    def convert_cairo_surface_to_photoimage(self, surface: cairo.ImageSurface) -> ImageTk.PhotoImage | None:
        """
        ...
        """
        if surface is None:
            print("convert_cairo_surface_to_photoimage: Received None surface.")
            return None
        ...
        try:
            data_buffer = surface.get_data() # Get data after checking surface is not None
        except Exception as e: # Underlying surface might be bad (e.g. after PDF error)
            print(f"Error getting data from Cairo surface: {e}")
            pil_image = Image.new("RGB", (max(1, width), max(1, height)), color="purple") # Use max(1,...) for 0-size
        ...
        try:
            if cairo_format == cairo.FORMAT_ARGB32:
                pil_image = Image.frombytes("RGBA", (width, height), data_buffer.tobytes(), "raw", "BGRA", stride)
            elif cairo_format == cairo.FORMAT_RGB24:
                pil_image = Image.frombytes("RGB", (width, height), data_buffer.tobytes(), "raw", "BGRX", stride)
            else:
            ...
            print(error_msg)
            pil_image = Image.new("RGB", (max(1,width), max(1,height)), color="orange")
            from PIL import ImageDraw # Local import for error case
            temp_draw = ImageDraw.Draw(pil_image)
    ...
    def debug_current_item(self, event=None, combined_debug=False):
        ...
        try:
            if not combined_debug:
                if self.navigation_mode == "page":
                    if self.current_page_number > 0:
                        self.update_status_bar(f"Debugging Page {self.current_page_number}...")
                        surface = self.engine.render_pdf_page(self.current_page_number, debug=self.engine.M_DEBUG)
                        general_debug_message = f"Debugged Page {self.current_page_number}."
                    else:
                        general_debug_message = "No page selected to debug."
                elif self.navigation_mode == "question":
                    self.update_status_bar("Debugging Questions (extraction)...")
                    self.questions_list = self.engine.extract_questions_from_pdf(debug=self.engine.M_DEBUG)
                    self.total_questions = len(self.questions_list) if self.questions_list else 0
                    if self.current_question_number > self.total_questions or \
                       (self.current_question_number == 0 and self.total_questions > 0):
                         self.current_question_number = 1 if self.total_questions > 0 else 0

                    general_debug_message = "Debugged Questions (extraction)."
                    if self.current_question_number > 0 and self.total_questions > 0:
                        general_debug_message += f" Current Q{self.current_question_number}."
                ...
                self.update_status_bar("Combined Debug: Extracting questions...")
                self.questions_list = self.engine.extract_questions_from_pdf(debug=self.engine.M_DEBUG)
                self.total_questions = len(self.questions_list) if self.questions_list else 0
                if self.current_question_number > self.total_questions or \
                   (self.current_question_number == 0 and self.total_questions > 0):
                ...
                page_to_debug = 1
                if self.navigation_mode == "page" and self.current_page_number > 0:
                    page_to_debug = self.current_page_number
                elif self.navigation_mode == "question" and self.current_question_number > 0 and self.questions_list:
                    try:
                        current_q = self.questions_list[self.current_question_number - 1]
                        if current_q.pages:
                            page_to_debug = current_q.pages[0]
                        else:
                            print(f"Warning: Q{self.current_question_number} has no 'pages'. Defaulting page 1 for debug.")
                    except IndexError:
                        print(f"Error: Q_idx {self.current_question_number} out of bounds. Defaulting page 1 for debug.")
                    except AttributeError:
                         print(f"Error: Question object missing 'pages'. Defaulting page 1 for debug.")

                self.update_status_bar(f"Combined Debug: Rendering page {page_to_debug}...")
                surface = self.engine.render_pdf_page(page_to_debug, debug=self.engine.M_DEBUG)
            ...
            if surface:
                ...
                self.display_canvas.coords(self.canvas_image_item, 0, 0)
                self.display_canvas.config(scrollregion=self.display_canvas.bbox(self.canvas_image_item))
            elif not (not combined_debug and self.navigation_mode == "question"):
                self.display_canvas.itemconfig(self.canvas_image_item, image=None)
                self._photo_image_ref = None
                self.display_canvas.config(scrollregion=(0,0,0,0))
            ...
            if combined_debug:
                 if not surface: # If combined debug didn't produce a direct page surface to display.
    ...
    def reload_engine_code(self, event=None):
        ...
        original_scaling = self.engine.scaling # Assuming scaling is an attribute

        if self.engine.current_pdf_document and self.engine.get_current_file_path():
        ...
        try:
            ...
            if all_pdf_paths:
                ...
                if current_pdf_path and current_pdf_idx != -1 : # Check if a PDF was actually loaded
                    ...
                    if self.engine.proccess_next_pdf_file(): # This should load the PDF at current_pdf_idx
                        print(f"Successfully reloaded and processed: {self.engine.get_current_file_path()}")
                        self.total_pages = self.engine.get_num_pages() if self.engine.current_pdf_document else 0

                        if current_mode == "page":
                            self.navigation_mode = "page"
                            self.current_page_number = min(current_page, self.total_pages) if self.total_pages > 0 else 0
                            if self.current_page_number == 0 and self.total_pages > 0: self.current_page_number =1
                            ...
                            self.current_question_number = min(current_question, self.total_questions) if self.total_questions > 0 else 0
                            if self.current_question_number == 0 and self.total_questions > 0: self.current_question_number = 1

                        # render_current_page_or_question will call update_status_bar with item details.
                        self.render_current_page_or_question()
                        self.update_status_bar(f"Engine reloaded. Restored state for {os.path.basename(current_pdf_path)}.")
                    else:
                        self.render_current_page_or_question()
                        self.update_status_bar("Engine reloaded. Could not restore previous PDF.")
                else:
                     self.render_current_page_or_question()
                     self.update_status_bar("Engine reloaded. No active PDF to restore. File list restored if any.")
            else:
                self.render_current_page_or_question()
                self.update_status_bar("Engine reloaded. No previous PDF list to restore.")
    ...
    def next_pdf_file(self):
        ...
        if self.engine.proccess_next_pdf_file():
            ...
            self.update_status_bar("End of PDF list.") # Explicitly set general message
        self.update_all_button_states()
    ...
    def previous_pdf_file(self):
        ...
        if self.engine.current_pdf_index > 0:
            ...
            success = self.engine.initialize_file(self.engine.all_pdf_paths[self.engine.current_pdf_index])
            self.navigation_mode = "page"
            if success:
                self.total_pages = self.engine.get_num_pages() if self.engine.current_pdf_document else 0
                self.current_page_number = 1 if self.total_pages > 0 else 0
    ...
    def switch_to_page_mode(self, event=None):
        ...
        # render_current_page_or_question will call update_status_bar with item details
        self.render_current_page_or_question()
        self.update_status_bar("Switched to Page Mode.") # General confirmation
        self.update_all_button_states()
        ...
        if self.engine.current_pdf_document:
            try:
                ...
                if self.total_questions == 0:
                    ...
                    self.update_status_bar(f"Switched to Question Mode. {self.total_questions} questions found.")
            except Exception as e:
    ...
    def next_item(self):
        ...
        if changed:
            ...
            self.update_status_bar(f"Already at the last {self.navigation_mode}.")
        self.update_all_button_states()
    ...
    def previous_item(self):
        ...
        if changed:
            ...
            self.update_status_bar(f"Already at the first {self.navigation_mode}.")
        self.update_all_button_states()
    ...
    def update_all_button_states(self):
        ...
        # PDF Navigation Buttons
        self.next_pdf_button.config(state=tk.NORMAL if num_files > 0 and current_pdf_idx < num_files - 1 else tk.DISABLED)
        self.prev_pdf_button.config(state=tk.NORMAL if num_files > 0 and current_pdf_idx > 0 else tk.DISABLED)

        # Mode Switching Buttons
        self.page_mode_button.config(state=tk.DISABLED if self.navigation_mode == "page" else tk.NORMAL)
        self.question_mode_button.config(state=tk.DISABLED if self.navigation_mode == "question" else tk.NORMAL)
        ...
        if pdf_loaded:
            if self.navigation_mode == "page":
                self.next_item_button.config(state=tk.NORMAL if self.current_page_number < self.total_pages else tk.DISABLED)
                self.prev_item_button.config(state=tk.NORMAL if self.current_page_number > 1 else tk.DISABLED)
            elif self.navigation_mode == "question":
                self.next_item_button.config(state=tk.NORMAL if self.current_question_number < self.total_questions else tk.DISABLED)
                self.prev_item_button.config(state=tk.NORMAL if self.current_question_number > 1 else tk.DISABLED)
