...
class QuestionDetectorV2(BaseDetector):
    def __init__(self) -> None:
        ...
        self.x_tolerance_pixels = (
            10  # a base tolerance for x-position matching
        )
    ...
    def handle_sequence(self, seq: SymSequence, page: int):
        ...
        if page != self.curr_page:
            if self.curr_page != -1:  # If it's not the very first page
                ...
                for level_idx in range(LEVEL_QUESTION, LEVEL_SUBPART + 1):
                    ...
                    if active_item:
                        ...
                        ):  # self.height is from previous page
                            # Ensure its end_page is the previous page
                            if (
                                self.curr_page not in active_item.pages
                            ):  # Should be previous page number
                                active_item.pages.append(
                                    self.curr_page
                                )  # Mark it as ending on self.curr_page (previous)
            ...
            previous_page_height_for_spanning_y1_calc = (
                self.height
            )  # store old height if needed for complex spanning
            self.curr_page = page
            ...
            for level_idx in range(LEVEL_QUESTION, LEVEL_SUBPART + 1):
                active_item = self.current_levels[level_idx]
                if (
                    active_item and active_item.end_page != page
                ):  # If it was not ended on previous page by a new sibling
                    if (
                        page not in active_item.pages
                    ):  # If it's truly spanning to this new page
                        active_item.pages.append(page)
                    active_item.y1 = (
                        self.height
                    )  # Extend to bottom of current new page by default
        ...
        for level in range(LEVEL_QUESTION, LEVEL_SUBPART + 1):
            ...
            for sub_seq in seq.iterate_split(
                " \t"
            ):  # Original QuestionDetector uses " \t"
    ...
    def _process_sub_sequence(
        self, sub_seq: SymSequence, level: int, page: int
    ) -> bool:
        accumulated_chars = ""
        first_char_props = None  # To store x, y, h of the first valid char of a potential marker

        if not sub_seq:  # Skip empty sub_sequences
            return False

        for sym_idx, sym in enumerate(sub_seq):
            # Basic X validation
            ...
            if self._is_skip_char(sym.ch, level):
                ...
                if (
                    sym.ch != "." or sym_idx < len(sub_seq) - 1
                ):  # if "." is not last, or other skip char
                    potential_text_with_skip_char = (
                        accumulated_chars + sym.ch
                    )  # check if it forms a valid marker with the skip char
                    is_marker, marker_label, marker_type = (
                        self._check_potential_marker(
                            potential_text_with_skip_char,
                            level,
                            (
                                first_char_props["x"]
            ...
            is_marker, marker_label, marker_type = (
                self._check_potential_marker(
                    accumulated_chars, level, first_char_props["x"]
                )
            )
            ...
            if is_marker:
                ...
                self._update_question_structure(
                    q_base, level, page, first_char_props["x"]
                )
    ...
    def _get_marker_type_and_cleaned_label(
        ...
        if m:
            ...
            if checkIfRomanNumeral(
                m.group(1).upper()
            ):  # checkIfRomanNumeral expects uppercase
                ...
                if checkIfRomanNumeral(cleaned_roman_text):
                    return (
                        ROMAN,
                        cleaned_roman_text.lower(),
                    )  # Store roman in lowercase consistent with other types
        ...
        if checkIfRomanNumeral(
            cleaned_text_for_roman.upper()
        ):  # try uppercase for safety
            return (
                ROMAN,
                cleaned_text_for_roman.lower(),
            )  # Store roman in lowercase

        return UNKNOWN, None

    def _check_potential_marker(
        self, text: str, level: int, x_pos: float
    ) -> tuple[bool, str | None, int | None]:
        marker_type, marker_label = self._get_marker_type_and_cleaned_label(
            text, level
        )
        ...
        if expected_type_for_level == UNKNOWN:
            ...
            if level == LEVEL_QUESTION:
                ...
                if (
                    not self.question_list
                    and marker_type != NUMERIC
                    and marker_label != "1"
                ):  # First question in doc must be "1"
                ...
                if self.current_levels[level - 1] is None:
                    ...
                    return (
                        ...
                        None,
                        None,
                    )  # Cannot have a part without a question, or subpart without part.
            ...
            if isinstance(
                expected_label_for_level, list
            ):  # E.g. after "1", next could be "a" or "i"
                ...
                expected_label_for_level is not None
                and marker_label != expected_label_for_level
            ):
    ...
    def _is_x_pos_valid(self, x_pos: float, level: int) -> bool:
        if not self.level_x_zones[
            level
        ]:  # No markers confirmed for this level yet on this page/context
        ...
        average_x = sum(self.level_x_zones[level]) / len(
            self.level_x_zones[level]
        )
        # Tolerance can increase slightly for sub-levels, or be a config parameter
        allowed_delta = self.x_tolerance_pixels * (
            1 + level * 0.5
        )  # e.g. L0: 10px, L1: 15px, L2: 20px
    ...
    def _update_question_structure(
        self, new_q_base: QuestionBase, level: int, page: int, x_pos: float
    ):
        ...
        if (
            previous_sibling and previous_sibling.y1 == self.height
        ):  # Only update if it was extending to page bottom
        ...
        if level == LEVEL_QUESTION:
            ...
            if parent_question:
                ...
                print(
                    f"WARN: LEVEL_PART '{new_q_base.label}' found without parent question. Page: {page}, X: {x_pos}"
                )
            ...
            if parent_part:
                ...
                print(
                    f"WARN: LEVEL_SUBPART '{new_q_base.label}' found without parent part. Page: {page}, X: {x_pos}"
                )
        ...
        if (
            current_item_at_this_level
            and current_item_at_this_level != new_q_base
        ):  # Ensure not same object
            if (
                current_item_at_this_level.end_page == new_q_base.start_page
                and current_item_at_this_level.y1 == self.height
            ):
        ...
        if new_q_base.q_type is not None and new_q_base.label is not None:
            ...
            self.level_expected_labels[level] = get_next_label(
                new_q_base.label, new_q_base.q_type
            )
            ...
            print(
                f"WARN: new_q_base '{new_q_base.label}' has no q_type for level {level}. Expectations not set."
            )
        ...
        for deeper_level in levels_to_reset:
            ...
            if (
                self.current_levels[deeper_level]
                and self.current_levels[deeper_level].end_page
                == new_q_base.start_page
                and self.current_levels[deeper_level].y1 == self.height
            ):
            ...
            self.level_x_zones[deeper_level] = (
                []
            )  # Clear x-zones for deeper levels as context changes
        ...
        for q_base in self.question_list:
            ...
            for part in q_base.parts:
                ...
                for subpart in part.parts:
                    if subpart.y1 is None:
                        subpart.y1 = self.height
    ...
    def on_finish(self):
        ...
        for level_idx in range(LEVEL_QUESTION, LEVEL_SUBPART + 1):
            ...
            if active_item:
                ...
                if (
                    active_item.y1 == self.height
                ):  # Check if it extends to current (last) page's bottom
                ...
                if active_item.end_page != self.curr_page:
                    ...
                    if self.curr_page not in active_item.pages:
                        active_item.pages.append(
                            self.curr_page
                        )  # Ensure its page range includes the last page
                ...
                if active_item.y1 is None or (
                    active_item.y1 is not None
                    and active_item.y1 < active_item.y
                    and active_item.start_page == active_item.end_page
                ):
                    active_item.y1 = self.height
