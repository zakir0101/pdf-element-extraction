...
class PdfEngine:
    ...
    def get_page_stream(self, page_number: int):
        ...
        self.renderer = BaseRenderer(self.state)
    ...
    def debug_original_stream(self, filename="output\\original_stream.txt"):
        with open(filename, "w", encoding="utf-8") as f:
            pprint.pprint(self.pages[self.current_page - 1], f)
            self.print_fonts(f)
            self.print_external_g_state(f)
            f.write(self.current_stream)
        return self

    def print_external_g_state(self, f):
        exgtate = {}
        ext = (
        ...
        with open("output\\output.md", "w", encoding="utf-8") as f:
            ...
            for cmd in self.parser.parse_stream(stream).iterate():
                ...
                if cmd.name in ["Tj", "TJ", "'", '"']:
                    ...
                    if counter > max_show:
                        break
        self.renderer.save_to_png("output\\output.png")
